...
const collection_account_seeds = [
	Buffer.from("collection"),
	mintCollection.toBuffer(),
];

const [collection_account_key, _collection_account_bump] =
	PublicKey.findProgramAddressSync(
		collection_account_seeds,
		program.programId
	);

const mintTs = Keypair.generate();

const initializeTxn = async () => {
	try {
		const txhash = await program.methods
			.initialize("collinsezedike")
			.accountsPartial({
	...
	try {
		const txhash = await program.methods
			...
			.accountsPartial({
				...
				authority: collection_account_key,
